...
abstract class CatalogRemoteDataSource {
  ...
  Future<List<DiscountModel>> getWorkshopDiscounts(int businessId);
}
...
class CatalogRemoteDataSourceImpl implements CatalogRemoteDataSource {
  ...
  @override
  Future<List<DiscountModel>> getWorkshopDiscounts(int businessId) async {
    try {
      final response =
          await apiRequest.get(EndPoints.workshopDiscounts(businessId));
      if (response.statusCode != 200) {
        throw ServerException(
          message: getErrorMessage(response.statusCode ?? 0),
        );
